    ...
    private fun save() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val recId = appDb.shReceiveDao().insert(shReceive)

                val tempDetailList = appDb.tempShReceiveDetailDao().getAll()
                val detailList = ShReceiveDetail.transformFromTempWithShReceiveId(recId, tempDetailList)
                appDb.shReceiveDetailDao().insert(detailList)

                val tempMortalityList = appDb.tempShReceiveMortalityDao().getAll()
                val mortalityList = ShReceiveMortality.transformFromTempWithShReceiveId(recId, tempMortalityList)
                ...
                val printText = printModule.constructReceivePrintout(recId)

                withContext(Dispatchers.Main) {
                    wm.enqueueUpload(ShReceive.TABLE_NAME, recId)
                    findNavController().navigate(ReceConfFragmentDirections.actionReceConfFragmentToPrintPreviewFragment(printText))
                }
